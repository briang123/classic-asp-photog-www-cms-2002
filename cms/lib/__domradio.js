...
for (var i=0;i<qArr.length;i++) {
	switch(qArr[i]) { //get the querystring keyname and set the keyvalue by performing lookup on our copied array
		...
		case 'cn':categoryName=lookupArr[i+1];break;
...
function toggleMenuImage(oid,srcImg,trgImg) {
	if (isAll || isID) {
		...
		(dom.src==strCmsImagePath+srcImg) ? dom.src=strCmsImagePath+trgImg : dom.src=strCmsImagePath+srcImg;
...
function viewConnectedPhotos() {
	...
	for(var i=0;i<cnxnList.options.length;i++) { 
		...
		if (categoryId>0) {
			listHtml+='<img align=\"middle\" style=\"cursor:pointer;cursor:hand;border:1 solid #666;\" vspace=\"1\" width=\"50\" height=\"50\" src=\"'+strPortfolioRootPath+photographer+'/'+categoryName+'/thumbs/'+thumbImg+'.jpg\"></a>&nbsp;'			
		} else if(galleryId>0) {
			listHtml+='<img align=\"middle\" style=\"cursor:pointer;cursor:hand;border:1 solid #666;\" vspace=\"1\" width=\"50\" height=\"50\" src=\"'+strProofRootPath+photographer+'/'+galleryLastName+'/thumbs/'+thumbImg+'.jpg\"></a>&nbsp;'
		}
...
function radClick(rad,imgname) {
	...
	backDrop.backgroundImage ='url('+strCmsImagePath+'ConnectedBackDrop.gif)';
